...
- [ ]  고객에게 메뉴판을 출력한다.
- [ ]  사용자로부터 메뉴와 개수를 입력받는다.
- [ ]  사용자에게 주문 메뉴, 할인 전 총 주문 금액, 증정 메뉴, 혜택 내역, 총 혜택 금액, 할인 후 예상 결제 금액, 이벤트 배지 순으로 출력한다.
- [ ]  해당 사항이 없는 건 ‘없음’으로 표시한다.

## 어플리케이션

- [ ]  총주문 금액 10,000원 이하일 경우 할인 X
- [ ]  크리스마스 할인: 1000원으로 시작해서 12월 25일까지 매일 100원씩 고정 할인 금액 증가
- [ ]  12월 31일까지 평일 할인(일,월,화,수,목)은 디저트 메뉴를 메뉴 1개당 2023원 할인
- [ ]  12월 31일까지 주말 할인(금,토)은 메인 메뉴를 메뉴 1개당 2023원 할인
- [ ]  12월 31일까지 특별 할인(3일,10일,17일,24일,25일,31일)은 총 금액에서 1000원 할인
- [ ]  할인 전 총 주문 금액이 12만원 이상일 경우 샴페인 1개 증정
- [ ]  총 혜택 금액에 따라 이벤트 배지 (5천원 이상: 별, 1만원 이상: 트리, 2만원 이상: 산타) 증정

## 예외

- [x]  방문 날짜가 1이상 31이하의 숫자가 아닐 경우 예외
- [ ]  음료만 주문할 경우 주문X
- [x]  메뉴가 20개 이상일 경우 예외 발생
- [x]  메뉴판에 없는 메뉴를 주문할 시 예외 발생
- [x]  메뉴의 개수가 1이상의 숫자가 아닐 경우 예외 발생
- [x]  메뉴 형식이 예외와 다를 경우 예외 발생
- [x]  중복 메뉴를 입력할 경우 예외 발생
- [x]  아무것도 입력하지 않았을 경우 예외 발생
